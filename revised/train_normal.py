...
def main():
    ...
    for x in ['AP2DC','AD2D','EState','CDKExt','CDK','CDKGraph','KRFPC','KRFP','MACCS','PubChem','SubFPC','SubFP', 'Combined']:
        ...
        for model_prefix, model_instance in model_types:
            ...
            results_file = "results_cv_normal.csv"
            ...
            results_loocv_file = "results_loocv_normal.csv"
    ...
    for fp in fingerprints:
        ...
        for model_name, model in base_model_defs:
            ...
            m = clone(model)
            m.fit(X_fp, y)
            # Use the full-model predictions as stacking features
            if hasattr(m, "predict_proba"):
                stacked_features[col_name] = m.predict_proba(X_fp)[:, 1]
            else:
                stacked_features[col_name] = m.decision_function(X_fp)
